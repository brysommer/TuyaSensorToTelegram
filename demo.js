...
const getRequestSign = async (
  ...
  const sortedQuery = Object.keys(queryMerged)
    .sort()
...
const getToken = async () => {
  ...
  const { data } = await httpClient.get("/v1.0/token?grant_type=1", {
    headers,
  });
...
const getDeviceInfo = async () => {
  ...
  if (!data?.success) {
    bot.sendMessage(config.chatId, "❄️ УВАГА: МОРОЗІЛКА ОФЛАЙН! ❄️");
  }

  const voltageStatus = data.result.find(
    (status) => status.code === "cur_voltage"
  );
  if (voltageStatus) {
    ...
    if (voltage > 215) message = `🟢 Напруга в нормі: ${voltage.toFixed(1)}V`;
      ...
      message = `🔴 Дуже низька напруга: ${voltage.toFixed(1)}V`;
    else if (voltage < 215)
      message = `🟡 Низька напруга: ${voltage.toFixed(1)}V`;
...
const monitorVoltage = async () => {
  try {
    await getToken();
    await getDeviceInfo();
    setInterval(getDeviceInfo, 60 * 60 * 1000);
    const now = new Date();
    const millisTill10AM =
      new Date(
        now.getFullYear(),
        now.getMonth(),
        now.getDate(),
